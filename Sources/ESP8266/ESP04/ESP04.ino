...
bool wifiConnectTry;
unsigned long wifiDisconnectionLapse;
...
void loop () {
  ...
  switch ( wifiStatus ){
    ...
    default:{
      ...
      if ( wifiStatusChanged ){
        wifiConnectTry=false;
        //acaba de perderse la conexion
        //asignamos un contador de tiempo
        wifiDisconnectionLapse = millis();
      }else{
        if ( !wifiConnectTry ){
          wifiConnectTry = true;
        ...
        if (millis() - wifiDisconnectionLapse > 60000 ){
          ESP.reset();          
        }
      }
    }break;
