...
void loop() {
  ...
  while ( SERIAL_ESP.available() ){
    ...
    if (reciveCommand && command=="PING"){  }
    if (reciveCommand && command=="ESPRST"){ ping=false; cambio=true; reciveCommand=false; command = ""; }
    if (reciveCommand && command=="OK" ){
      for ( d=0; d<DEVICES; d++){
        Devices[d]->notified = false;
      }
      cambio=false;
    ...
    //en caso de que se reciba un comando incorrecto que no ha sido detectado
    if ( reciveCommand ){
      reciveCommand=false;
      command = "";
    }
